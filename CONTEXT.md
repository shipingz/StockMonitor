...
| ADR-0011 | 抓取容错：适度并发（FETCH_WORKERS 默认 3，1~8 可调）+ 每线程限速（threading.local）+ 指数退避重试 3 次 | 已确认 |
...
| ADR-0016 | 实时价检测模式：任意时刻可触发；判定「上一根已收盘K线收盘价 ≤ MA60 < 实时价」（锚点法无状态）；接受重复推送；需交易时段护栏 | 已确认（待实现） |
